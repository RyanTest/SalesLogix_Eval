...
{this.ClientId=clientId;this.PanelId=clientId+"_Panel";this.InitCall=initCall;this.LookupControls=new Array();this.CurrentIndex=0;this.Size=size+"px";this.AutoPostBack=autoPostBack;this.panel=null;}
...
{if(this.CanShow())
{if((this.panel==null)||(this.panel.element.parentNode==null))
{var lookup=document.getElementById(this.PanelId);lookup.style.display="block";this.panel=new YAHOO.widget.Panel(this.PanelId,{visible:false,width:this.Size,fixedcenter:true,constraintoviewport:true,underlay:"shadow",draggable:true,iframe:true,modal:false});this.panel.render();if((this.CurrentIndex==0)||(this.LookupControls[i]!=undefined))
{this.Init();}
for(var i=0;i<this.CurrentIndex;i++)
{if(this.LookupControls[i]!=undefined)
{var text=document.getElementById(this.LookupControls[i].TextId);var seedVal="";if(i>0)
{seedVal=this.GetSeeds(i);}
if((text.value!="")||(seedVal!="")||(i==0))
{this.LookupControls[i].CurrentValue=text.value;var listId=this.LookupControls[i].ListId;var list=document.getElementById(listId);if(list.options.length==0)
...
{var dependCtrl=new dependControl(baseId,listId,textId,type,displayProperty,seedProperty);this.LookupControls[this.CurrentIndex]=dependCtrl;this.CurrentIndex++;}
function DependencyLookup_AddFilters(FilterProp,FilterValue)
{var dependCtrl=new dependControl(listId,textId,type,displayProperty,seedProperty);this.LookupControls[this.CurrentIndex]=dependCtrl;this.CurrentIndex++;}
function DependencyLookup_SelectionChanged(index)
{if((index+1)<this.CurrentIndex)
{this.LookupControls[index+1].LoadList(this.GetSeeds(index+1));for(var i=index+2;i<this.CurrentIndex;i++)
{this.LookupControls[i].ClearList();}}}
function DependencyLookup_Ok()
{this.panel.hide();for(var i=0;i<this.CurrentIndex;i++)
{var text=document.getElementById(this.LookupControls[i].TextId);var list=document.getElementById(this.LookupControls[i].ListId);if((list.selectedIndex!=undefined)&&(list.selectedIndex!=-1))
...
{var result="";for(var i=index;i>0;i--)
{var dependParent=this.LookupControls[i-1];var dependChild=this.LookupControls[i];var list=document.getElementById(dependParent.ListId);if(list.selectedIndex==-1)
{return result;}
...
DependencyLookup.prototype.CanShow=DependencyLookup_CanShow;DependencyLookup.prototype.Show=DependencyLookup_Show;DependencyLookup.prototype.AddControl=DependencyLookup_AddControl;DependencyLookup.prototype.SelectionChanged=DependencyLookup_SelectionChanged;DependencyLookup.prototype.Ok=DependencyLookup_Ok;DependencyLookup.prototype.Init=DependencyLookup_Init;DependencyLookup.prototype.InvokeChangeEvent=DependencyLookup_InvokeChangeEvent;DependencyLookup.prototype.GetSeeds=DependencyLookup_GetSeeds;dependControl=function(baseId,listId,textId,type,displayProperty,seedProperty)
{this.BaseId=baseId;this.ListId=listId;this.TextId=textId;this.Type=type;this.DisplayProperty=displayProperty;this.SeedProperty=seedProperty;this.CurrentValue="";}
...
var vURL="SLXDependencyHandler.aspx?cacheid="+this.BaseId+"&type="+this.Type+"&displayprop="+this.DisplayProperty+"&seeds="+seedValue+"&currentval="+this.CurrentValue;xmlhttp.open("GET",vURL,false);xmlhttp.send(null);this.HandleHttpResponse(xmlhttp.responseText);}
dependControl.prototype.HandleHttpResponse=function(results)
{var list=document.getElementById(this.ListId);list.innerHTML="";var items=results.split("|");for(var i=0;i<items.length;i++)
...
{var list=document.getElementById(this.ListId);list.innerHTML="";}

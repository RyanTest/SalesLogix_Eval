...
Ext.extend(Ext.ux.tree.PagingTreeLoader, Ext.tree.TreeLoader, {
    ...
    doPreload : function(node){
		...
		if (node.id != 'root')
		{node.attributes.pagingInfo = pi = {limit: 999,start: 0};}
    ...
    processResponse : function(response, node, callback){
        ...
        try {
			...
			if(pi.limit < pi.total && (node.id == 'root') ){
				this.createToolbar(node);
			}
...
Ext.ux.tree.PagingTreeToolbar = Ext.extend(Ext.Toolbar, {
	...
	autoCreate: {
		...
		style:'',
	    html:'<table cellspacing="0" style="width:200px;padding:2px;height:25px;background-color:#D5E2F2;"><tr></tr></table>'
	},
	
	// private
    onRender : function(ct, position){
		this.height = Ext.isIE ? 30 : 30;
		this.width = 200;
